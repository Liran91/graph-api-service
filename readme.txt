...
Once a request to get the graph is made, we fetch the graph routes and nodesData and then filter the graph according to the filter type (if a valid filter type was supplied) - the graph is returned unfiltered.
...
I've chosen an Adjacency List as my graph data structure because it is more efficicent for traversal algorithms which I am using (DFS).
